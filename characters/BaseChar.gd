...
func block_hitbox(hitbox, force_parry = false, force_block = false, ignore_guard_break = false, autoblock_armor = false):
		...
		if perfect_parry:
			...
			if not projectile and not perfect_parry and not last_turn_block and initiative:
				if hitbox.guard_break and not ignore_guard_break:
		...
		if not projectile:
			...
			if not current_state() is GroundedParryState:
		...
		if current_state() is GroundedParryState:
			current_state().anim_name = "ParryLow" if (hitbox.hit_height == Hitbox.HitHeight.Low or not is_grounded()) else "ParryHigh"
		...
		if not perfect_parry:
			...
			if not projectile:
				...
				if opponent.feints < opponent.num_feints:
					opponent.feints += 1
				if not (hitbox.looping and not hitbox.cancellable) and hitbox.block_cancel_allowed:
