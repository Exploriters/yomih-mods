...
const SCRATCHES_SCENE_S2 = preload("res://!Just-someModdingCharacters/Characters/bloodypotato/particles/scratchesS2.tscn")
...
func _frame_9():
	...
	length = abs(sqrt(pow(float(vec_x) , 2) + pow(float(vec_y), 2))) * 0.5
	#print(length)
...
func _on_hit_something(obj, hitbox):
	...
	if host.combo_count > 0:
		...
		st_pos_x = posP.x
		st_pos_y = posP.y
		vec2_x = tp_pos_x - st_pos_x
		vec2_y = tp_pos_y - st_pos_y
		...
		spawn_particle_relative(SCRATCHES_SCENE_S2, Vector2(particle_position.x + 32 * (host.get_opponent_dir()) , particle_position.y - 10), Vector2(vec2_x, vec2_y))
		#scratches.emission_rect_extents = length
		#scratches.position = -length
